...
TailMeth1 <- read.csv("p01_iapglobal_final011416.csv")
...
TailMeth2 <- na.omit(TailMeth1[,c("sample_id","Litter", "id", "Sex","Exposure.Group","Exposure_Coded","Age.Group","Age", "luke_age", "iap_avg_all4","iap1avg", 
                                  "iap2avg","iap3avg","iap4avg")]) 
detach(TailMeth1)
attach(TailMeth2) 
detach(TailMeth2)

## characteristics at baseline 
summary(iap_avg_all4[Age==1])
summary(iap_avg_all4[Age==0]) ##distribution of methylation at baseline

##Histograms to examine normality
par(mfrow=c(2,1)) ##Makes 2x1 frame histogram. 
hist(iap_avg_all4,nclass=20) ##nclass=20 indicates that you want 20 bars. 

par(mfrow=c(2,2))
hist(iap1avg,nclass=20)
hist(iap2avg,nclass=20)
hist(iap3avg,nclass=20)
hist(iap4avg,nclass=20)

par(mfrow=c(1,1))
#hist(log(iap_avg_all4),nclass=20) #Checking whether log-transformation helps with normality. In this case, it looks fairly normal before transformation.
##Note: Repeated measures, so log transormation is probably not appropriate here.

##Scatterplots to look at spread of data
plot(Exposure_Coded,iap_avg_all4) ##Scatterplot exposure vs. PND22 methylation 
plot(Exposure_Coded,iap1avg) ##Scatterplot exposure vs. Site 1 methylation

boxplot(iap_avg_all4 ~ Exposure.Group, main = "Absolute IAP Methylation by Exposure Group", 
...
##Model 1 - 12/08/15
TailMeth.lmer<-lmer(iap_avg_all4~Age+Age*factor(Exposure.Group1)+Age*factor(Sex)+(1|id)+(1|Litter), data=TailMeth2) 
summary(TailMeth.lmer)

##Wald test for directionality of effects.

##Model 2 - 12/08/15
TailMeth.lmer1<-lmer(iap_avg_all4~Age+(1|id)+(1|Litter), data=TailMeth2) 
summary(TailMeth.lmer1)

##Model 3 - 12/18/15 -- TESTED on 12/18/15; adding factor(Exposure.Group1) does not make a difference for beta/p-values!
TailMeth.lmer3<-lmer(Mean.Methylation~Age+factor(Exposure.Group1)+Age*factor(Exposure.Group1)+factor(Sex)+Age*factor(Sex)+(1|id)+(1|Litter), data=TailMeth2) 
summary(TailMeth.lmer3)
...
difflsmeans(TailMeth.lmer, test.effs=NULL)
