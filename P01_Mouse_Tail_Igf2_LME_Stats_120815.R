...
TailMeth1 <- read.csv("mIgf2_Tail_Methylation_GEE Formatted_120715.csv")
...
boxplot(Mean.Methylation ~ Exposure.Group, main = "Absolute Igf2 Methylation by Exposure Group", 
...
##Model 1 - 11/17/15
TailMeth.lmer<-lmer(Mean.Methylation~Age+Age*factor(Exposure.Group1)+Age*factor(Sex)+(1|id)+(1|Litter), data=TailMeth2) 
summary(TailMeth.lmer)

TailMeth.lmer2<-lmer(Mean.Methylation~factor(Exposure.Group1)+(1|id)+(1|Litter), data=TailMeth2) 
summary(TailMeth.lmer2)
